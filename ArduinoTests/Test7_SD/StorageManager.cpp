...
void StorageManager::init() {
    Logger::info("Initializing SD Card Subsystem...");
    ...
    if (!SD.begin(SD_CS_PIN)) {
        ...
        stor.totalSpaceMB = SD.totalBytes() / (1024 * 1024);
        stor.usedSpaceMB = SD.usedBytes() / (1024 * 1024);
        stor.freeSpaceMB = stor.totalSpaceMB - stor.usedSpaceMB;
        
        Logger::info("SD Size: " + String(stor.totalSpaceMB) + "MB");
    }
