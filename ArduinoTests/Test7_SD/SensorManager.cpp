...
void SensorManager::init() {
    ...
    Logger::info("Initializing Temp/Hum Sensor (DHT22)...");
    // dht.begin();
...
static float _readHardwareTemperature() { return 0.0; }
static float _readHardwareHumidity() { return 0.0; }
...
float SensorManager::readBattery() {
    ...
    float voltage = CalibrationManager::applyBatteryCalibration(adc);
    float pct = CalibrationManager::calculateBatteryPercentage(voltage);
    ...
    if (voltage < 0.0f || voltage > 5.0f || adc > 4095 || pct < 0.0f || pct > 100.0f) {
        ...
        delay(50);
        adc = _readBatteryADC();
        voltage = CalibrationManager::applyBatteryCalibration(adc);
