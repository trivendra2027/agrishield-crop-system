"""
PlantCLEF Dataset Downloader & Integrator
Downloads the PlantCLEF dataset from Kaggle, verifies, deduplicates, and merges into combined_dataset.
"""
import os
import sys
import json
...
logger = logging.getLogger("PlantCLEF_Downloader")
...
PLANTCLEF_DIR = os.path.join(WORKSPACE, "datasets", "PlantCLEF")
...
KAGGLE_REFS = [
    "datajameson/planclef",
    "amboarajesde/split-plantclef",
    "ghousiah/plantclef"
]
...
def download_plantclef():
    """Download PlantCLEF dataset from Kaggle."""
...
def integrate(dl_root):
    ...
    for root, dirs, files in os.walk(dl_root):
        ...
        if img_files and len(img_files) >= 5:
            # Skip splits like 'train', 'val', 'test' - we want their children
            basename = os.path.basename(root).lower()
            if basename in ('train', 'val', 'valid', 'test', 'images', 'data'):
                pass # usually classes are *inside* these.
                     # But if the leaf folder is named train, we just use its parent.
    ...
    for cls_root, img_files in class_dirs:
        ...
        if folder_name.lower() in ('train', 'val', 'valid', 'test', 'images', 'data'):
            cls_name = f"PlantCLEF_{os.path.basename(os.path.dirname(cls_root))}"
        else:
            cls_name = folder_name.replace(" ", "_").replace("-", "_")

        target_dir = os.path.join(PLANTCLEF_DIR, cls_name)
        ...
        for fname in img_files:
            ...
            dst_target = os.path.join(target_dir, fname)
...
def update_reports(new_classes, images_added):
    ...
    with open(os.path.join(REPORTS_DIR, "dataset_statistics.json"), "w") as fp:
        json.dump({"total_classes": len(active), "total_images": total_images,
                    "empty_classes": len(missing), "new_from_plantclef": len(new_classes),
                    "images_from_plantclef": images_added}, fp, indent=2)
    ...
    if not any("PlantCLEF" in (s.get("name", s) if isinstance(s, dict) else s) for s in sources["sources"]):
        sources["sources"].append({"name": "PlantCLEF Dataset", "type": "Kaggle",
...
def main():
    log("=" * 70)
    log("PLANTCLEF DATASET DOWNLOAD & INTEGRATION — START")
    log("=" * 70)
    t0 = time.time()
    os.makedirs(PLANTCLEF_DIR, exist_ok=True)

    dl_path = download_plantclef()
    if not dl_path:
        log("ERROR: Could not download PlantCLEF dataset. Aborting.")
        return

    new_classes, added, corrupt, dups = integrate(dl_path)

    size_bytes = sum(os.path.getsize(os.path.join(root, f)) for root, _, files in os.walk(PLANTCLEF_DIR) for f in files)
    ...
    summary = (
        f"\n{'='*70}\n"
        f"PLANTCLEF INTEGRATION — COMPLETE\n"
        f"{'='*70}\n"
        ...
        f"• PlantCLEF storage used:              {size_gb} GB\n"
        f"• Storage location:                    datasets/PlantCLEF/\n"
