"""
Rice Disease Dataset Downloader & Integrator
Downloads multiple Rice Disease datasets from Kaggle, verifies, deduplicates, and merges into combined_dataset.
"""
import os
import sys
import json
...
logger = logging.getLogger("RiceDisease_Downloader")
...
RICE_DIR = os.path.join(WORKSPACE, "datasets", "RiceDisease")
...
KAGGLE_REFS = [
    ...
    "minhhuy/rice-diseases-image-dataset",
    "shibumohapatra/rice-leaf-diseases",
    "shounakdey/rice-diseases-image-dataset",
    "jillanisofttech/rice-leaf-disease-dataset"
]
...
def normalize_rice_class(folder_name):
    """Normalize common rice disease names to standard taxonomy."""
    name = folder_name.lower().replace("_", " ").replace("-", " ")
    if "blast" in name and "leaf" in name:
        return "Rice_Blast"
    elif "blast" in name:
        return "Rice_Blast"
    elif "brown" in name and "narrow" in name:
        return "Rice_Narrow_Brown_Spot"
    elif "brown" in name:
        return "Rice_Brown_Spot"
    elif "blight" in name and "sheath" in name:
        return "Rice_Sheath_Blight"
    elif "blight" in name and "bacterial" in name:
        return "Rice_Bacterial_Leaf_Blight"
    elif "blight" in name:
        return "Rice_Bacterial_Leaf_Blight"
    elif "smut" in name and "false" in name:
        return "Rice_False_Smut"
    elif "smut" in name and "leaf" in name:
        return "Rice_Leaf_Smut"
    elif "smut" in name:
        return "Rice_Leaf_Smut"
    elif "tungro" in name:
        return "Rice_Tungro"
    elif "bakanae" in name:
        return "Rice_Bakanae"
    elif "scald" in name:
        return "Rice_Leaf_Scald"
    elif "hispa" in name:
        return "Rice_Hispa"
    ...
    # Fallback formatting
    return "Rice_" + folder_name.replace(" ", "_").replace("-", "_").title()


def integrate():
    """
    Download and integrate all Rice datasets. Verify, deduplicate, merge.
    """
    ...
    for ref in KAGGLE_REFS:
        ...
        try:
            ...
            for cls_root, img_files in class_dirs:
                ...
                cls_name = normalize_rice_class(folder_name)

                rice_cls = os.path.join(RICE_DIR, cls_name)
                combined_cls = os.path.join(COMBINED_DIR, cls_name)
                os.makedirs(rice_cls, exist_ok=True)
                ...
                for fname in img_files:
                    ...
                    dst_rice = os.path.join(rice_cls, fname)
                    dst_cb = os.path.join(combined_cls, fname)
                    if not os.path.exists(dst_rice):
                        shutil.copy2(src, dst_rice)
...
def update_reports(new_classes, images_added):
    ...
    with open(os.path.join(REPORTS_DIR, "dataset_statistics.json"), "w") as fp:
        json.dump({"total_classes": len(active), "total_images": total_images,
                    "empty_classes": len(missing), "new_from_rice": len(new_classes),
                    "images_from_rice": images_added}, fp, indent=2)
    ...
    if not any("Rice Disease" in (s.get("name", s) if isinstance(s, dict) else s) for s in sources["sources"]):
        sources["sources"].append({"name": "Rice Disease Datasets", "type": "Kaggle",
...
def main():
    log("=" * 70)
    log("RICE DISEASE DATASETS DOWNLOAD & INTEGRATION — START")
    log("=" * 70)
    t0 = time.time()

    new_classes, added, corrupt, dups = integrate()

    size_bytes = sum(os.path.getsize(os.path.join(root, f)) for root, _, files in os.walk(RICE_DIR) for f in files)
    ...
    summary = (
        f"\n{'='*70}\n"
        f"RICE DISEASE INTEGRATION — COMPLETE\n"
        f"{'='*70}\n"
        ...
        f"• Rice storage used:                   {size_gb} GB\n"
        f"• Storage location:                    datasets/RiceDisease/\n"
